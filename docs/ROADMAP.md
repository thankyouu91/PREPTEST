...
- [ ] Ngữ pháp mệnh đề quan hệ và mệnh đề phụ, bậc B2 (8 điểm còn lại theo hạn mức: rút gọn mệnh đề quan hệ bằng V-ing / V3 / to-V, giới từ + which và whom, mệnh đề quan hệ với lượng từ (`some of which`, `most of whom`), `which` thay cho cả mệnh đề đứng trước, nhượng bộ nâng cao (`whereas`, `while`), mệnh đề chỉ cách thức (`as if`, `as though`), `whenever`/`wherever`/`whoever`, mệnh đề danh ngữ với `what` và `whether`)
