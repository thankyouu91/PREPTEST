...
- [ ] VPET exam engine: per-part timer, audio playback with a fixed replay count, microphone capture for parts H/I/J, autosave and submit
  - [ ] Attempt metering, once the engine exists: `codes.attempts_used` is stored and surfaced everywhere but nothing decrements it, because nothing creates a sitting yet. Until then Starter's "practice stops after the tenth sitting" is a stated intent, not an enforced rule. **Blocked on the engine above — do not take it on its own**
