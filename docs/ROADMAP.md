...
- [ ] **Translate `scripts/`, slice 2: the admin and account harnesses** — `test-admin.mjs` (235 lines), `test-user-api.mjs` (100), `test-taikhoan.js` (55), `tai-khoan.js` (46). Watch the CSV column contract (`ky_thi`, `ky_nang`, …): those are a format identifiers asserted against the downloadable template, and renaming them is a functional change, not a translation
...
- [ ] **Rename the Vietnamese filenames** — `tai-khoan.js`, `test-taikhoan.js`, `xuat-supabase.mjs`. Identifiers are covered by "English everywhere", but a rename touches `verify.sh`, `package.json` and the docs that mention them, so it is a separate step from translating their contents
