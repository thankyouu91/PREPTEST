...
- [ ] **Translate slice 4b: the `/api/admin/*` messages in `server/api.js`** — 218 Vietnamese lines, of which about 65 are error strings the admin screens now display in Vietnamese against an English interface; the rest are comments. Split out of 4a deliberately rather than rushed at the end of a long turn: five keyed replacements in 4a truncated mid-string and had to be repaired, and that is exactly the failure this file cannot afford, since one assertion (`test-admin.mjs:589`, `/chưa sẵn sàng/`) reads one of these messages
