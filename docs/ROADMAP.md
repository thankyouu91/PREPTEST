...
- [ ] **VPET exam runner, the screen** — the candidate-facing half: part list and entry, live countdown fed by `secondsLeft`, audio player that spends a replay and shows how many are left, microphone capture for parts H/I/J via MediaRecorder, autosave on a timer and on blur, submit with a confirmation. The backend above is complete and tested; this item is only the interface over it
...
- [ ] Translate the whole interface to English — 12 student screens, 8 admin screens, every banner and empty state
  - [ ] Shop and price-list copy is the first screen to translate: plan names, perks and limits already arrive from `plans.js` in English while the words around them are Vietnamese, so it reads bilingual today. **Part of the pass above — translating this screen alone is the half-done translation the standing rules forbid**
