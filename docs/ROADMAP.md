...
- [ ] **`test-auth.mjs` is flaky, and it gates every push.** Seen twice on 2026-08-12, both times with code that passed the same suite minutes earlier, so it is timing and not a regression. It dies at line 184 with `page.fill('#email')` timing out on `/prep/dang-nhap/`. The mechanism: the password-change at line 177 is followed by a fixed `waitForTimeout(1200)`. Under load that POST has not landed yet, so the *old* password at line 182 still works, the client redirects to `/prep/`, and line 184's visit to the sign-in page is bounced by `guestPage()` to `/prep/` — where there is no `#email` to fill, and Playwright waits out its 30 seconds. The suite is full of these fixed waits (900 ms, 1200 ms) standing in for "the request finished". Replace them with waits on the outcome — the success banner, the URL, a response promise — because a gate that fails for reasons unrelated to the change teaches everyone to re-run it until it is green, which is the same as having no gate. Also worth doing in the same pass: `npm run verify` now takes over fifteen minutes, and the 900-second ceiling a caller might reasonably put around it kills the browser mid-screenshot
