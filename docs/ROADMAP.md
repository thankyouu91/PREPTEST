...
- [ ] **Translate `scripts/`, slice 3: the browser-driven scripts** — `test-auth.mjs` (132), `test-exam.mjs` (174), `test-catalog.mjs` (49), `audit.mjs` (28), `screenshot.mjs` (28), `shot-admin.mjs` (4), `xuat-supabase.mjs` (45). These assert on product copy in places, so each string needs checking against what the interface actually renders
