...
- [x] **Security sweep across the whole API** — `server/security.js`, `docs/SECURITY.md`, `scripts/security-map.mjs` and `scripts/test-security.mjs` (47 checks). The lasting part is not the fixes, it is that the table is **generated from the Express stack** and re-generated on every `npm run verify`: adding an endpoint without a guard is red immediately, rather than waiting for the next sweep. Reading the stack rather than the source matters here — `router.use('/admin', requireAdmin, csrfGuard)` wraps only the routes registered *after* it, and reading the file makes it easy to conclude the opposite. What the sweep found and fixed: base security headers were on HTML responses only, so every API response and every static asset went out bare — they are now set once, globally, before any router, with `Permissions-Policy` denying everything except `microphone=(self)`, which the speaking parts need for MediaRecorder; HSTS is sent only when the request already arrived over HTTPS, because sending it in development pins `localhost` to HTTPS in the developer's own browser for a year; `/api/*` now carries `default-src 'none'; frame-ancestors 'none'; sandbox`, since an API response is data and never a document; and a global write limit (`WRITE_PER_MIN`, default 300/min, keyed on the **hashed session cookie × IP** so a stolen session is the unit being counted) sits under every mutating route including ones written tomorrow. The eight write endpoints that legitimately have no auth guard are named in `PUBLIC_WRITES` with what stands in for one, and the test checks that list in both directions — an undeclared one is red, and so is a stale entry
  - [ ] **Issue `prep_csrf` before sign-in, so login CSRF closes.** The one real hole the sweep found and did not fix. Six unauthenticated write endpoints have no `csrfGuard`, because the cookie is only minted on a successful sign-in and a guest has nothing to match against. Registration, forgot-password and the token routes gain nothing for an attacker, but **login** does: forcing a victim's browser to sign in as the attacker means the victim's subsequent work is recorded in the attacker's account. The fix is to mint `prep_csrf` when the page is served rather than when the session starts. Kept out of the sweep because it touches all four auth screens and their tests, and a security change bundled into a large diff is a security change nobody reviews
- [x] **`test-auth.mjs` was flaky, and it gates every push** — seventeen fixed `waitForTimeout` calls replaced by waits on the outcome, and three per-window rate limits made configurable so the suite can be run repeatedly at all. Proved rather than asserted: **twelve consecutive runs, all 53/53**, where before this turn the sixth run in an hour could not pass. Four distinct causes, and only the first was the one originally diagnosed:
    - **The clock race.** The password change was followed by `waitForTimeout(1200)`. Under load the POST had not landed, so the *old* password still worked two lines later, the page went inside, and the next visit to the sign-in screen was bounced by `guestPage()` — leaving Playwright waiting out 30 seconds for an `#email` that could not exist. Every fixed wait is now a wait on the operation reaching a terminal state, then an assertion about which state. Waiting for exactly the thing you are about to assert makes the assertion vacuous; waiting for "it has settled either way" does not
    - **Settling on the wrong element.** My own first attempt at the fix waited for `#step-done` on the forgot-password screen and then clicked `#dev-link-a`, which appears a beat later — removing the fixed wait while keeping the race. Worth recording because it is the obvious way to get this wrong
    - **A placeholder read too early.** `#greet-name` ships with the literal text `Student` and is overwritten when `/api/me` answers. The old fixed wait hid this; waiting properly for the dashboard to render exposed it. The test was reading the placeholder and comparing it to the real name
    - **The suite ate its own rate limits.** Every run spends one registration, one password-reset request and one code redemption. `REGISTER_PER_HOUR` was already configurable for exactly this reason; `FORGOT_PER_HOUR` (5/hour) and `REDEEM_PER_10MIN` (12/10 min) were not, so a sixth run in an hour failed on quota. Both now read from the environment with their production defaults unchanged, and `verify.sh` raises all three. This is the one that matters most for maintenance: a suite that cannot be run repeatedly is a suite whose flakiness cannot be investigated
  - [ ] **`npm run verify` takes over fifteen minutes**, and most of it is the screenshot step walking 43 pages serially in one browser. A caller who wraps the command in a 900-second timeout — a reasonable thing to do — kills the browser mid-screenshot and gets a Playwright crash instead of a result. Run the pages across a few contexts concurrently, and reuse one browser rather than launching per script. Split out of the flake fix because it is a different job with a different risk
