...
- [ ] **Translate `scripts/`, slice 4: `test-learn.mjs`** — 581 lines on its own, and the only file where the two languages genuinely have to coexist: the self-study teaching content it asserts on **stays Vietnamese** by standing decision, while the test's own prose becomes English. Deserves its own turn for exactly that reason
