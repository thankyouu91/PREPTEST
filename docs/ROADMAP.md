...
- [ ] **Candidate result report** ([`docs/VOICE.md`](VOICE.md) §8.2): GSE 10–90 per skill beside the CEFR band, distance to the next boundary, and ranked "tips to max score" built from the lowest criteria, the part H word alignment, the deterministic speech metrics, part J content coverage and the model's evidence quotes — each tip linked to the matching `/prep/hoc/` study page at the learner's level. Releases immediately for practice tests; flagged answers show as "needs a teacher" rather than a fabricated score
