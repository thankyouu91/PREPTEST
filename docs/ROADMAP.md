...
- [x] Tag items by VPET part: `questions.part` (A-J) so each part draws from its own pool instead of sharing one skill-wide pool — column and AI-drafted items carry it; the form generator still needs to filter on it
...
- [ ] **One-touch form build**: chain part blueprint → script → MP3 → items → bank in a single run, with a review screen per part before anything commits. The pieces all exist; what is missing is the orchestrator and the queue behind it
...
- [ ] Batch render across a whole form, behind the `media_jobs` queue — one item at a time is right for authoring, a 37-file form is not ([`docs/VOICE.md`](VOICE.md) §7)
...
- [ ] **Two exam levels** ([`docs/VOICE.md`](VOICE.md) §1.7): `tests.level` 1 or 2, items tagged by level so a Level 1 form never draws Level 2 items, and a scoring engine that clamps to the level's measurement range — reporting a ceiling or floor with a recommendation to sit the other level instead of a number the form cannot support
- [ ] **Result code** ([`docs/VOICE.md`](VOICE.md) §8.5): `attempts.result_code` in the existing `makeCode()` alphabet with an `R-` prefix, minted at submit and never reissued; admin and candidate lookups route by code rather than numeric id so the string on the paper is the string that works everywhere; `/verify/:code` public and rate limited, returning verification detail only
- [ ] **Candidate result report** ([`docs/VOICE.md`](VOICE.md) §8.2): GSE 10–90 per skill beside the CEFR band, distance to the next boundary, and ranked "tips to max score" built from the lowest criteria, the part H word alignment, the deterministic speech metrics, part J content coverage and the model's evidence quotes — each tip linked to the matching `/prep/hoc/` study page at the learner's level. Carries the result code and the learner's GSE line across previous attempts. Releases immediately for practice tests; flagged answers show as "needs a teacher" rather than a fabricated score
