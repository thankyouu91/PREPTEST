...
- [ ] **Rename the Vietnamese identifiers** — the filenames `tai-khoan.js`, `test-taikhoan.js`, `xuat-supabase.mjs`; the CLI verbs that tool accepts (`xem`, `dat-lai-admin`, `dat-lai-student`, `mo-khoa`); and the local variable names inside the already-translated files (`lenh`, `soAdmin`, `daLech`, `chay`, `chon`, `matKhau`, …). All three kinds are covered by "English everywhere", and all three were deliberately left out of the translation slices for the same reason: they are renames with call sites — `verify.sh`, `package.json`, the README table, `cai-dat/tai-khoan.bat` — rather than prose, and mixing a rename into a translation makes both harder to review
