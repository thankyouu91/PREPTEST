...
- [ ] **Translate slice 2: the landing page** — the last pre-login screen still in Vietnamese, so the seam today is landing → sign in. Biggest single file of marketing copy in the project (~3k characters), which is why it is on its own rather than rushed in behind the account screens
- [ ] **Translate slice 3: the signed-in student area** — shared chrome (`_chrome.js`), dashboard, library, test detail, exam runner, result, the three code screens, account, and the client strings in `_mock.js`, together with the `/api/redeem` and `/api/attempts/*` messages. This has to move as one piece: the chrome is shared, so translating it alone leaves every screen bilingual
  - [ ] Shop and price-list copy belongs to slice 3: plan names, perks and limits already arrive from `plans.js` in English while the words around them are Vietnamese
