...
- [ ] Shop copy follow-up: the shop now sells the three plans, but the surrounding copy is still Vietnamese while the plan names, perks and limits come from `plans.js` in English. The screen reads bilingual until the interface is translated
- [ ] Attempt metering: Starter's ten sittings are stored (`codes.attempts_used`) and surfaced everywhere, but nothing decrements them yet — that lands with the exam engine, which is what creates a sitting. Until then Starter's "practice stops after the tenth sitting" is a stated intent, not an enforced one
- [ ] Landing prices are hand-typed in `public/prep/landing/index.html` while every other screen reads them from `plans.js`. The copy around them is tuned Vietnamese and worth keeping, but the numbers should be written in from `PrepState.plans()` after boot — otherwise the next price change leaves the shop-window lying
