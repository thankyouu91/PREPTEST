...
| Mệnh đề quan hệ, mệnh đề phụ | `/prep/hoc/menh-de/` | 9 điểm bậc A2–B1. Mệnh đề quan hệ: `who`/`which`/`that`, `whose`, `where`/`when`/`why`, mệnh đề xác định khác không xác định (**hai dấu phẩy đổi cả nghĩa câu**), và lược bỏ đại từ — bỏ được khi nó làm tân ngữ, không bỏ được khi làm chủ ngữ. Mệnh đề trạng ngữ: thời gian (**không dùng `will` sau `when`**), nguyên nhân, mục đích, nhượng bộ. Hai mục riêng trị hai lỗi dấu vân tay của người Việt: `Because… so…` và `Although… but…` (tiếng Việt nói đủ *Vì… nên…*, tiếng Anh chỉ được một). Bậc B2 và C1–C2 soạn ở lượt sau |
