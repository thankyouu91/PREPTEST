...
| Câu điều kiện | `/prep/hoc/dieu-kien/` | 11 điểm bậc A2–B2: loại 0–1–2–3, điều kiện hỗn hợp, `unless` và nhóm liên từ điều kiện, `wish`/`if only` ở hiện tại và quá khứ, `would rather`, `it's time`, `supposing`/`otherwise`. Trọng tâm là cơ chế lùi thì để đánh dấu chuyện không có thật — nguồn của hai lỗi kinh điển `if … will` và `if … would`. Bậc C1–C2 soạn ở lượt sau |
