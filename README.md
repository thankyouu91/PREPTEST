...
| Động từ khuyết thiếu | `/prep/hoc/khuyet-thieu/` | 14 điểm bậc A1–B1: `can`, `could`, `must`, `have to`, `should`, `may`/`might`, `would like`, `used to`, `be able to`. Trọng tâm là sắc thái — `mustn't` là CẤM còn `don't have to` là KHÔNG CẦN; suy đoán phủ định dùng `can't` chứ không phải `mustn't`; không ghép hai khuyết thiếu (`will can` sai). Bậc B2–C2 soạn ở lượt sau |
