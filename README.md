...
| `npm run docs:vpet` | dựng bộ tài liệu VPET từ `server/data/exam-formats.js` ra `public/tai-lieu/`: `guide.json` cho trang đọc trực tiếp `/prep/tai-lieu/`, cùng hai bản PDF Level 1 / Level 2. **Bắt buộc chạy + commit sau khi sửa cấu trúc đề** — máy chủ chỉ chạy `npm ci --omit=dev`, mà Playwright là devDependency nên nó không tự dựng lại được |
...
| `node scripts/test-paper.mjs` | **đề mà thí sinh thật sự ngồi làm**, đối chiếu với blueprint: đủ mười phần A-J, đúng số câu từng phần, mỗi phần chỉ bốc từ pool của chính nó, không câu nào nằm ở hai phần, phần có audio thì mọi câu phải có bản ghi. Chạy độc lập, **không cần server**. Nó đọc CSDL bằng kết nối riêng chứ không qua `server/db.js` — vì nạp `server/db.js` là chạy seed, mà seed nay tự sửa đề lệch: một phép kiểm tự chữa lành thứ nó đang đo thì không đo gì cả |
...
| `node scripts/test-pg-schema.mjs` | nạp lược đồ đã dịch sang **PostgreSQL thật** rồi so hai bên: 34 bảng, từng cột, từng NOT NULL, hai kiểu phải đổi, và các dạng câu lệnh mã nguồn đang dùng (`ON CONFLICT DO UPDATE`, khoá ngoại, index `DESC`). Không có `PG_URL` thì **bỏ qua và nói rõ**, không lặng lẽ xanh. `scripts/pg-dev.sh` dựng sẵn một cụm tạm — có **mật khẩu** (`scram-sha-256`), để cụm ở máy xác thực giống cụm thật và giống CI |
